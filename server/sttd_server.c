...
int sttd_server_start(int uid, const char* lang, const char* recognition_type, int silence, const char* appid)
{
	...
	int secu_ret = 0;
	secu_ret = security_server_check_privilege_by_pid(sttd_client_get_pid(uid), "stt-server", "rwx");
	SLOG(LOG_DEBUG, TAG_STTD, "[Server] pid(%d), security check ret(%d)", sttd_client_get_pid(uid), secu_ret);

	if (SECURITY_SERVER_API_SUCCESS != secu_ret) {
		SLOG(LOG_ERROR, TAG_STTD, "[Server ERROR] STT permission denied");
