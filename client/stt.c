...
int stt_create(stt_h* stt)
{
	...
	secu_ret = smack_have_access(label, "stt-server", "rwx");
	SLOG(LOG_DEBUG, TAG_STTC, "[Check] Subject smack label(%s), security check ret(%s)", label, secu_ret ? "success" : "fail");

	if (true != secu_ret) {
		SLOG(LOG_ERROR, TAG_STTC, "[ERROR] STT permission denied");
